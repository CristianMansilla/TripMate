...
create table public.expenses (
  ...
  notes text,
  optional boolean not null default false,
  occurrence_pricing text not null default 'total' check(occurrence_pricing in ('total','per_occurrence')),
  created_by uuid references auth.users(id),
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
);
...
create index activity_steps_activity_position_idx on public.activity_steps(activity_id, position);
...
create unique index reservations_one_per_trip_item on public.reservations(item_id);
...
begin
  ...
  if p_cost_mode='new' then
    ...
    if p_cost_amount_basis is null or p_cost_amount_basis not in ('per_person','group') then raise exception 'La base del costo no es válida.'; end if;
  end if;

  if p_activity_id is not null then
    select * into v_activity from public.activities where id=p_activity_id and trip_id=p_trip_id for update;
    if not found then raise exception 'No se encontró la actividad.'; end if;
    if v_activity.expense_id is not null then raise exception 'Esta actividad tiene un gasto vinculado. Editala desde Presupuesto.'; end if;
  ...
  if p_activity_id is null then
    ...
    update public.activities set
      ...
      actual_cost=case when p_cost_mode='new' and p_status='paid' then p_cost_amount when p_cost_mode='new' then null else actual_cost end,
...
create or replace function public.save_activity_plan_v2(
  ...
  p_title text, p_date date, p_start_time time, p_end_time time,
  p_category text, p_place text, p_notes text, p_status text, p_optional boolean,
  p_cost_mode text, p_cost_amount numeric, p_cost_amount_basis text, p_steps jsonb
) returns uuid
language plpgsql
security invoker
set search_path = public
...
begin
  if p_steps is not null and jsonb_typeof(p_steps)<>'array' then raise exception 'Las paradas no son válidas.'; end if;
  ...
  update public.activities set category=v_category where id=v_activity_id and trip_id=p_trip_id;
  ...
  for v_step in select value from jsonb_array_elements(coalesce(p_steps,'[]'::jsonb)) loop
    ...
    if v_step_id is null then
      insert into public.activity_steps(trip_id,activity_id,title,amount,start_time,end_time,place,notes,optional,position,created_by)
      values(p_trip_id,v_activity_id,trim(v_step->>'title'),coalesce(nullif(v_step->>'amount','')::numeric,0),
        v_step_start,v_step_end,nullif(trim(coalesce(v_step->>'place','')),''),nullif(trim(coalesce(v_step->>'notes','')),''),
        coalesce((v_step->>'optional')::boolean,false),v_position,auth.uid()) returning id into v_step_id;
    else
      update public.activity_steps set title=trim(v_step->>'title'),start_time=v_step_start,end_time=v_step_end,
  ...
  delete from public.activity_steps where activity_id=v_activity_id and (cardinality(v_kept_ids)=0 or not(id=any(v_kept_ids)));
  return v_activity_id;
end;
$$;
...
begin
  ...
  if p_item_id is null then
    ...
    insert into public.trip_items(id,trip_id,title,category,place,notes,optional,origin_type,origin_id,created_by)
    values(v_item_id,p_trip_id,trim(p_title),trim(p_category),nullif(trim(coalesce(p_place,'')),''),
      nullif(trim(coalesce(p_notes,'')),''),coalesce(p_optional,false),'item',v_item_id,auth.uid());
  else
    select * into v_item from public.trip_items where id=p_item_id and trip_id=p_trip_id for update;
    ...
    v_item_id:=v_item.id;
    update public.trip_items set title=trim(p_title),category=trim(p_category),
      place=nullif(trim(coalesce(p_place,'')),''),notes=nullif(trim(coalesce(p_notes,'')),''),
      optional=coalesce(p_optional,false) where id=v_item_id;
  end if;
  ...
  if p_expense is null then
    ...
    if v_expense_id is null then
      ...
      ) select p_trip_id,v_item_id,null,trim(p_title),trim(p_category),v_expense_amount,trip.currency,
      ...
      update public.expenses set activity_id=null,title=trim(p_title),category=trim(p_category),amount=v_expense_amount,
        ...
        place=nullif(trim(coalesce(p_place,'')),''),notes=nullif(trim(coalesce(p_notes,'')),''),optional=coalesce(p_optional,false)
      where id=v_expense_id;
    end if;
  end if;
  ...
  for v_activity in select value from jsonb_array_elements(coalesce(p_activities,'[]'::jsonb)) loop
    ...
    if v_date is null or not exists(select 1 from public.trips where id=p_trip_id and v_date between start_date and end_date) then
      raise exception 'Cada día del itinerario debe estar dentro de las fechas del viaje.';
    end if;
    ...
    if v_activity_id is null then
      insert into public.activities(
        ...
        nullif(trim(coalesce(p_place,'')),''),nullif(trim(coalesce(p_notes,'')),''),coalesce(v_expense_amount,0),
        case when v_expense_status='paid' then v_expense_amount else null end,
      ...
      update public.activities set expense_id=v_expense_id,date=v_date,start_time=v_start_time,end_time=v_end_time,
    ...
    for v_step in select value from jsonb_array_elements(coalesce(v_activity->'steps','[]'::jsonb)) loop
      ...
      if v_step_id is null then
        insert into public.activity_steps(trip_id,activity_id,title,amount,start_time,end_time,place,notes,optional,position,created_by)
        values(p_trip_id,v_activity_id,trim(v_step->>'title'),coalesce(nullif(v_step->>'amount','')::numeric,0),
          v_start_time,v_end_time,nullif(trim(coalesce(v_step->>'place','')),''),nullif(trim(coalesce(v_step->>'notes','')),''),
          coalesce((v_step->>'optional')::boolean,false),v_step_position,auth.uid()) returning id into v_step_id;
      else
        update public.activity_steps set title=trim(v_step->>'title'),amount=coalesce(nullif(v_step->>'amount','')::numeric,0),
          start_time=v_start_time,end_time=v_end_time,place=nullif(trim(coalesce(v_step->>'place','')),''),
          notes=nullif(trim(coalesce(v_step->>'notes','')),''),optional=coalesce((v_step->>'optional')::boolean,false),position=v_step_position
        where id=v_step_id and activity_id=v_activity_id and trip_id=p_trip_id;
        if not found then raise exception 'Una parada no pertenece a esta aparición.'; end if;
      end if;
      if v_step_id=any(v_kept_step_ids) then raise exception 'Hay una parada repetida en el formulario.'; end if;
    ...
    delete from public.activity_steps where activity_id=v_activity_id
      and (cardinality(v_kept_step_ids)=0 or not(id=any(v_kept_step_ids)));
    v_activity_position:=v_activity_position+1;
  end loop;
  delete from public.activities where item_id=v_item_id
    and (cardinality(v_kept_activity_ids)=0 or not(id=any(v_kept_activity_ids)));

  if v_expense_id is not null then
    update public.expenses expense set activity_id=v_first_activity_id,expense_date=activity.date,
      itinerary_start_time=activity.start_time,itinerary_end_time=activity.end_time
    from (select * from public.activities where id=v_first_activity_id) activity where expense.id=v_expense_id;
    if v_first_activity_id is null then
      update public.expenses set activity_id=null,expense_date=null,itinerary_start_time=null,itinerary_end_time=null
      where id=v_expense_id;
    end if;
  ...
  if p_reservation is null then
    ...
    if v_reservation_id is null then
      insert into public.reservations(trip_id,item_id,activity_id,expense_id,title,status,priority,due_date,notes,amount,position,created_by)
      values(p_trip_id,v_item_id,v_first_activity_id,v_expense_id,trim(p_title),v_reservation_status,v_priority,v_due_date,
        nullif(trim(coalesce(p_notes,'')),''),v_legacy_amount,
        coalesce((select max(position)+1 from public.reservations where trip_id=p_trip_id),0),auth.uid())
      returning id into v_reservation_id;
    else
      update public.reservations set activity_id=v_first_activity_id,expense_id=v_expense_id,title=trim(p_title),
        status=v_reservation_status,priority=v_priority,due_date=v_due_date,
        notes=nullif(trim(coalesce(p_notes,'')),''),amount=v_legacy_amount where id=v_reservation_id;
    end if;
  end if;
...
begin
  if not public.can_edit_trip(p_trip_id) then raise exception 'No tenés permiso para editar este viaje.'; end if;
  select updated_at into v_updated_at from public.trip_items where id=p_item_id and trip_id=p_trip_id for update;
  if not found then raise exception 'No se encontró el elemento del viaje.'; end if;
...
begin
  if tg_table_name='reservations' then
    ...
    if tg_op='UPDATE' and old.item_id is distinct from new.item_id then v_previous_item_id:=old.item_id; end if;
  end if;
  ...
  loop
    select min(activity.date) into v_first_activity_date from public.activities activity
    where activity.item_id=v_reservation.item_id;
    select trip.end_date into v_trip_end_date from public.trips trip where trip.id=v_reservation.trip_id;
...
create constraint trigger reservations_validate_due_date
  after insert or update or delete on public.reservations deferrable initially deferred
...
create constraint trigger activities_validate_reservation_due_date
  after insert or update or delete on public.activities deferrable initially deferred
  for each row execute function public.validate_trip_item_reservation_due_date();

-- Cualquier escritura realizada por las RPC anteriores invalida la version de
-- la ficha completa para que el editor unificado detecte cambios concurrentes.
...
create policy "members read activity steps" on public.activity_steps for select to authenticated using(
  public.is_trip_member(trip_id)
  and exists(select 1 from public.activities activity where activity.id=activity_steps.activity_id and activity.trip_id=activity_steps.trip_id)
);
create policy "editors write activity steps" on public.activity_steps for all to authenticated
