...
export default function ReservationDocumentUploadModal({
  file,travelerNames,onClose,onUpload,
}:{
  file:File
  ...
  async function submit(event:FormEvent){
    ...
    if(!passengerLabel){setMessage(selection===OTHER_TRAVELER?'Ingresá el nombre del pasajero.':'Seleccioná a quién corresponde el pasaje.');return}
  ...
  return <div className="modal-backdrop" onMouseDown={event=>{if(event.currentTarget===event.target)onClose()}}>
    <form ref={dialogRef} className="modal confirm-modal reservation-document-modal" role="dialog" aria-modal="true" aria-labelledby="reservation-document-title" tabIndex={-1} onSubmit={submit}>
      <ModalCloseButton onClick={onClose}/>
      <h2 id="reservation-document-title">Identificar pasaje</h2>
      <div className="document-file-summary"><FileText size={20}/><div><strong>{file.name}</strong><small>{fileSize}</small></div></div>
      <div className="field">
        <label htmlFor="reservation-document-passenger">Pasajero</label>
        <select id="reservation-document-passenger" value={selection} onChange={event=>{setSelection(event.target.value);setMessage('')}} autoFocus required>
      ...
      {selection===OTHER_TRAVELER&&<div className="field"><label htmlFor="reservation-document-other">Nombre del pasajero</label><input id="reservation-document-other" value={otherName} maxLength={80} onChange={event=>{setOtherName(event.target.value);setMessage('')}} autoFocus/></div>}
      {message&&<div className="field-error" role="alert">{message}</div>}
      <div className="modal-actions"><button className="btn btn-primary"><Upload size={16}/> Adjuntar PDF</button></div>
    </form>
  </div>
}
