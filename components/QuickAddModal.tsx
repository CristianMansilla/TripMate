...
type Kind='expense'|'reservation'|'packing'
...
export default function QuickAddModal({kind,onClose,onSave}:{kind:Kind,onClose:()=>void,onSave:(payload:any)=>Promise<void>|void}){
  ...
  const labels={expense:'Nuevo gasto',reservation:'Nueva reserva',packing:'Agregar a valija'} as const
  async function submit(e:FormEvent){
    e.preventDefault();setLoading(true)
    try{
      await onSave({title,amount:Number(amount||0),category,assignedTo,priority})
      onClose()
